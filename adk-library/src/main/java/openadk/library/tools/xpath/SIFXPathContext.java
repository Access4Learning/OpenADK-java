...
//			return null;
		...
		for( ; currentStep < steps.length; currentStep++  ){
			...
			if( nt instanceof NodeNameTest )
			{
            QName nodeName = ((NodeNameTest) nt).getNodeName();
                // if the current step contains the "and adk:x()" marker then create new element
                int index = 0;
                boolean andAdkX = hasAndAdkX(steps[currentStep]);
                if (andAdkX) {
                    NodeIterator ni = parent.childIterator(nt, false, null);
                    for (index = 0; index < 32; index ++) {
                        if (!ni.setPosition(index+1)) {
                            break;
                        }
                    }
                }
				current = parent.createChild( this, ((NodeNameTest) nt ).getNodeName(), index);
				if( current == null ){
    ...
    public boolean hasAndAdkX(Step step) {
        Expression[] expressions = step.getPredicates();
        for (Expression expression : expressions) {
            String str = expression.toString();
            if (str.endsWith(" and adk:x()")) {
                return true;
            }
        }
        return false;

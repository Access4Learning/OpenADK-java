...
public class SIFVersion implements Serializable, Comparable<SIFVersion>
{
    ...
    public static final String XMLNS_BASE="http://www.sifinfo.org/";
...
//// WARNING: MAKE SURE TO UPDATE THE GETINSTANCE METHOD WHEN ADDING NEW VERSIONS ////
	...
	public static SIFVersion parse( String versionStr )
	{
		try
		{
			...
			if( r == -1 ) {
				...
				if( minorStr.equals( "*" ) ){
                    throw new IllegalArgumentException( "Wildcard minor version ('" + versionStr + "') is not supported." );
				} else {
	...
	public String getXmlns()
	{
		...
		if(Main.getLocale().equals("uk"))
			return XMLNS_BASE + "uk/infrastructure/" + fMajor + ".x";
		
		if(Main.getLocale().equals("au"))
			return XMLNS_BASE + "au/infrastructure/" + fMajor + ".x";

		return XMLNS_BASE + "infrastructure/" + fMajor + ".x";
	}
